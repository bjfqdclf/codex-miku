Create a single horizontal sprite strip for the Codex app digital pet `hatsune-miku` in the state `running`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Create a Codex digital pet inspired by Hatsune Miku: compact chibi proportions, teal twin-tails, cheerful expressive face, black and gray singer outfit with small teal accents, tiny headset-like side details, no text or logos, no stage scenery. Keep it sprite-friendly and simplified..

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: Codex app built-in digital pet style: pixel-art-adjacent, thick dark 1-2 px outlines, stepped edges, limited palette, flat cel shading, compact readable silhouette, chroma-key background for cleanup..
...
Animation action: active working/in-progress loop: Miku typing on a tiny laptop computer.


State-specific requirements:
- Show the pet actively working or processing, as if running a task: seated or standing at a tiny laptop computer, focused posture, busy hands typing, subtle head bobbing, blinking, and small twin-tail sway.
- Include one small dark gray laptop computer directly in front of or touching the pet in every frame. Keep it simple, text-free, logo-free, and fully inside each frame slot. The laptop is the only new prop allowed for this requested action.
...
- Effects are allowed only when they are state-relevant, opaque, hard-edged, pixel-style, fully inside the same frame slot, and physically touching or overlapping the pet silhouette or laptop.
...
- Do not include text, labels, frame numbers, visible grids, guide marks, speech bubbles, thought bubbles, UI panels, code snippets, readable screen content, scenery, checkerboard transparency, white backgrounds, or black backgrounds.
